...
active term, course scope, and the next 7 days of events. This is what
makes answers grounded instead of guessed.
"""
...
def upcoming_events(db: DB, course_id: int | None, days: int = 7) -> list[dict]:
    """Next N days of assignments + exams + events as plain dicts."""
    now = _now().isoformat()
    ...
    rows = db.conn.execute(
        """SELECT a.id, c.code, a.title, a.due_at, a.status, a.weight,
           ...
           UNION ALL
           SELECT e.id, c.code, e.title, e.starts_at, NULL, e.weight, 'exam' AS kind
           ...
           UNION ALL
           SELECT ev.id, c.code, ev.title, ev.starts_at, NULL, NULL, ev.kind
    ...
    return [dict(r) | {"when": _fmt(r["due_at"] or r["starts_at"])} for r in rows]


def build_system_prompt(db: DB, course_id: int | None = None) -> str:
    now = _now()
...
Syllabus/outline may exist under the course content files.
"""

    events = upcoming_events(db, course_id)
    events_str = "\n".join(
        f"  {e['when']} — {e['kind']} {e['code']}: {e['title']}"
        for e in events
    ) or "  (none in the next 7 days)"

    return f"""You are HippoCampus, Nate's personal course assistant for Western University.
...
4. To change something (extend a due date, add a note/fact/event), use the
   mutate_* tools. All mutations are audited automatically.
5. If you don't know or the data is missing, say so plainly.
6. web_search is for outside-the-harness questions only; prefer harness data
