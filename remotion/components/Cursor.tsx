...
export const Cursor: React.FC<{
	...
	dwell?: number;
}> = ({ from, to, clickAt, dwell = 45 }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	const moveEnd = Math.max(1, dwell);
	const p = spring({
		frame: Math.min(frame, moveEnd),
		fps,
		...
		<>
			<div
				style={{
					...
					boxShadow:
						"0 2px 12px rgba(0,0,0,0.55), 0 0 0 3px rgba(139,92,246,0.95)",
					pointerEvents: "none",
				}}
			/>
			{showRipple && (
