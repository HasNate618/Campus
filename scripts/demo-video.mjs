...
async function injectCursor(page) {
	await page.addInitScript(() => {
		...
		Object.assign(c.style, {
			...
			borderRadius: "50%",
			boxShadow: "0 2px 10px rgba(0,0,0,0.55), 0 0 0 2px rgba(139,92,246,0.9)",
			pointerEvents: "none",
			zIndex: "2147483647",
			left: "0",
			top: "0",
...
async function run() {
	...
	await page.route("**/api/chat", async (route) => {
		...
		if (msg.includes("late penalties") || msg.includes("late penalty")) {
			...
			tokenText =
				"All set! I moved Assignment 1 — Control Flow from Aug 29 to Aug 31 and added your note 'approved per email 2026-09-08'. It's marked as extended and logged to the audit trail.";
			toolStart = { tool: "mutate_update_assignment", args: { id: 4 } };
			toolEnd = { tool: "mutate_update_assignment", result: { id: 4 } };
	...
	try {
		...
		await page.mouse.move(200, 380, { steps: 18 });
		await page.evaluate(({ x, y }) => window.__moveCursor?.(x, y), {
			x: 200,
			y: 380,
		});
		await page.waitForTimeout(300);
		await page.mouse.move(600, 540, { steps: 18 });
		await page.evaluate(({ x, y }) => window.__moveCursor?.(x, y), {
			x: 600,
			y: 540,
		});
		...
		log("goto /courses/1");
		await page
			.goto(`${DEMO_URL}/courses/1`, {
				waitUntil: "networkidle",
				timeout: 10000,
			})
			.catch(() =>
				page.goto(`${DEMO_URL}/courses/1`, { waitUntil: "domcontentloaded" }),
			);
		...
		if ((await syl.count()) > 0) {
			await syl.evaluate((el) => el.classList.add("__yellow"));
			await page.waitForTimeout(900);
			await syl.evaluate((el) => el.classList.remove("__yellow"));
		}
		...
		await page
			.locator('a:has-text("Assignments")')
			.first()
			.click()
			.catch(() => {});
		await page.waitForTimeout(800);
		...
		if ((await row.count()) > 0)
			await row.evaluate((el) => el.classList.add("__yellow")).catch(() => {});
		await page.waitForTimeout(900);
		// close
		...
		await page
			.screenshot({ path: join(VIDEO_DIR, "demo-error.png"), fullPage: true })
			.catch(() => {});
		throw e;
	} finally {
		await context.close();
	...
	try {
		...
		log("transcode");
		execSync(
			`ffmpeg -y -i "${raw}" -c:v libx264 -preset medium -crf 18 -pix_fmt yuv420p -r 60 -vf "scale=1920:1080:flags=lanczos" -c:a aac -b:a 128k -movflags +faststart "${mp4}"`,
			{ stdio: "inherit" },
		);
