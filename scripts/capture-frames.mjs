...
async function run() {
	...
	await page.route("**/api/chat", async (route) => {
		...
		let toolStart = {
			tool: "search_corpus",
			args: { query: "late penalties" },
		};
		const toolEnd = { tool: "search_corpus", result: { hits: 3 } };
		if (msg.includes("thrashing") || msg.includes("paging")) {
			text =
				'Thrashing is when the system spends more time paging than executing — Lecture 4 calls it "excessive page faults from an undersized working set" [Lecture 4 p.12]. I\'ve moved your review to Aug 31 — want a blind-graded 5-question check on working sets?';
	...
	// 1) Today — zoom 1, longer settle so digest/next-7-days renders distinctly
	...
	await page.screenshot({ path: join(OUT, "today.png"), fullPage: false });
	console.log("[capture] today.png");

	// 2) Course hub — Content tab selected, wait for syllabus visible
	await page.goto(`${DEMO_URL}/courses/1`, {
		waitUntil: "networkidle",
		timeout: 15000,
	});
	await page.evaluate(() => {
	...
	// ensure Content tab is active and syllabus row is visible
	await page
		.waitForSelector("text=Syllabus", { timeout: 6000 })
		.catch(() => {});
	await page.waitForTimeout(600);
	await page.screenshot({
		path: join(OUT, "course-hub.png"),
		fullPage: false,
	});
	console.log("[capture] course-hub.png");

	// 3) Chat empty — same route but wait for Ask placeholder BEFORE typing
	await page.goto(`${DEMO_URL}/courses/1`, { waitUntil: "networkidle" });
	await page.evaluate(() => {
		document.documentElement.style.zoom = "0.88";
	});
	await page
		.waitForSelector('textarea[placeholder*="Ask"]', { timeout: 6000 })
		.catch(() => {});
	await page.waitForTimeout(500);
	// ensure no assistant message is visible for empty state
	await page.evaluate(() => {
	...
	if ((await input.count()) > 0) {
		...
		await page
			.waitForSelector(".msg-assistant", { timeout: 10000 })
			.catch(() => {});
		await page.waitForTimeout(1200);
		await page.screenshot({
			path: join(OUT, "chat-answer.png"),
			fullPage: false,
		});
