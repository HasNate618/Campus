...
You are the **coding/implementer agent**. Canonical project dir: **`~/campus`** on the homeserver (`ssh home`). Nate may also keep a mirror elsewhere; **build and deploy on `home`**.
...
|------|-----------------|
| This repo | `~/campus` |
| Runtime course data (prod) | `{data_root}/` |
...
| Bifrost (LLM gateway) | `bifrost.local` |
| pdf-extractor | Docker `pdf-extractor` on `home` |
| Whisper / transcribe | Docker `whisper` (and/or Cohere transcribe container) |
| trawl (web search MCP) | `127.0.0.1:11236`, `trawl.local` |
| ntfy | `ntfy` container / usual example.lab route |
| Caddy | `*.example.lab` → services |
