...
Lecture recordings (later) →   /srv/homelab/school/…       →   RAG (H3)
...
3. sha256 change detection → `/srv/homelab/school/{term}/{code}/…`
...
- **trawl** — default web search (`trawl.home.lab` / host `:11236`)
...
- Deploy: NixOS Docker on `home`, Caddy `school.home.lab`, data `/srv/homelab/school/`.
