...
export const api = {
	...
	courses: (activeOnly = true) =>
		get<Course[]>(`/courses?active_only=${activeOnly}`),
	...
	contentTree: (id: number) =>
		get<{
			nodes: ContentNode[];
			files: FileRecord[];
			file_topics: { file_id: number; topic_id: number }[];
		}>(`/courses/${id}/content-tree`),
	...
	assignment: (courseId: number, id: number) =>
		get<Assignment>(`/courses/${courseId}/assignments/${id}`),
	workspaceTree: (courseId: number) =>
		get<WorkspaceTree>(`/courses/${courseId}/workspace/tree`),
	workspaceRead: (courseId: number, path: string) =>
		get<{ text: string | null; viewable: boolean; asset: string | null }>(
	...
	syncLog: (runId: number) =>
		get<{ markdown: string }>(`/sync/runs/${runId}/log`),
	...
	chatSessions: (courseId?: number) =>
		get<ChatServerSession[]>(
			`/chat/sessions${courseId != null ? `?course_id=${courseId}` : ""}`,
		),
	chatSessionCreate: (courseId: number | null, title: string) =>
		post<ChatServerSession>("/chat/sessions", { course_id: courseId, title }),
	chatSessionGet: (id: number) =>
		get<ChatServerSession>(`/chat/sessions/${id}`),
	chatSessionSave: (
		id: number,
		body: {
			title?: string;
